...
def generate_sample(targets_path, img_name, templates, probabilities_vector, positions_list, images_out_path,
    ...
    if load_path is None:
        ...
        template_ids_selected[:1] = np.random.choice(list(range(2, 12)), size=1, replace=False)
        ...
        while total < nb_signs_in_img:
            ...
            template_small_category = templates[template_ids_selected[total]][3]
            ...
            if bbox:
                ...
                bbox['small_category'] = template_small_category
                bboxes.append(bbox)
        ...
        blur_value = float(np.random.uniform(0, 2)) * scale
    ...
    for bbox in bboxes:
        ...
        sw = (bbox['xmax'] - bbox['xmin'])/w
        sh = (bbox['ymax'] - bbox['ymin'])/h
        binary_line = "{} {} {} {} {}".format(bbox['small_category'], x_center, y_center,
            sw, sh)
        multiclass_line = "{} {} {} {} {}".format(
            bbox['category'], x_center, y_center, sw, sh)
    ...
    d_out_path = os.path.join(data_out_path, "{:05d}_{}.txt".format(nb_imgs_generated,
                                                                        os.path.splitext(img_name)[0]))
    
    with open(d_out_path, "w") as data_out_f:
        data_out_f.write('\n'.join(multiclass_annotation_lines))

    d_out_path = os.path.join(sdata_out_path, "{:05d}_{}.txt".format(nb_imgs_generated,
                                                                        os.path.splitext(img_name)[0]))
...
if __name__ == '__main__':
    ...
    images_out_path = os.path.join(args.out_path, "images")
    data_out_path = os.path.join(args.out_path, "bigLabels")
    sdata_out_path = os.path.join(args.out_path, 'smallLabels')
    os.makedirs(args.out_path)
    os.makedirs(images_out_path)
    os.makedirs(data_out_path)
    os.makedirs(sdata_out_path)
    ...
    category_id = 0
    data = {
        'train': 'ImageSet/train',
        'val': 'ImageSet/val',
        'test': 'ImageSet/test',
        'nc': len(template_names),
        'names': []
    }
    big_ls, small_ls = [], []
    for template_name in sorted(template_names):
        ...
        # Add alpha channel (fully opaque)
            alpha_channel = np.ones((template.shape[0], template.shape[1], 1), dtype=template.dtype)
            template = np.concatenate([template, alpha_channel], axis=2)

        category = os.path.splitext(template_name)[0]
        b, s = category.split('_')[:2]
        if b != '字':
            template = cv2.resize(template, (template.shape[1]//2, template.shape[0]//2), interpolation=cv2.INTER_AREA)
        else:
            template = cv2.resize(template, (template.shape[1]*50//template.shape[0], 50), interpolation=cv2.INTER_AREA)
        template_mask = get_mask_from_image(template)
        
        try:
            big_id = big_ls.index(b)
        except ValueError:
            big_ls.append(b)
            big_id = len(big_ls) - 1
            print(b + ':')
        small_ls.append(s)
        print(s, category_id)
        templates.append((template, template_mask, big_id, category_id))
        category_id += 1
    data['names'] = big_ls
    data['nc'] = len(big_ls)
    with open('ImageSet/bigLabel_data.yaml', 'w') as f:
        yaml.dump(data, f, allow_unicode=True)
    data['names'] = small_ls
    data['nc'] = len(small_ls)
    with open('ImageSet/smallLabel_data.yaml', 'w') as f:
        yaml.dump(data, f, allow_unicode=True)
    ...
    for idx, img_name in enumerate(img_names):
        p = mp.Process(target=generate_sample, args=((
            ...
            binary_annotation_lines_queue, multiclass_annotation_lines_queue, data_out_path, sdata_out_path, load_path)))
        p.daemon = True
        p.start()
        processes.append(p)
